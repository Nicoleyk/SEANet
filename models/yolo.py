...
from models.GFPN.RF_gn import RF_D_gn
from models.GFPN.Enhancement_module import FBSIE,CSPStageEM
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            ELAN1, RepNCSPELAN4, SPPELAN,CSPStageEM,RF_D_gn}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, SPPCSPC,CSPStageEM}:
